...
import { getExpectedExercisesForDate, getWeekDates, formatDate } from '../../utils/date';
...
export default function CoachMemberTrendsPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { members, measurements, trainingPlans, checkins } = useStore();
  ...
  const weeklyCompletionRates = useMemo(() => {
    if (memberMeasurements.length === 0) return [];
    const minDate = memberMeasurements[0].date;
    const maxDate = memberMeasurements[memberMeasurements.length - 1].date;
    const result: { weekStart: string; completionRate: number; midDate: string }[] = [];
    let cursor = new Date(minDate);
    const endDate = new Date(maxDate);
    while (cursor <= endDate) {
      const weekDates = getWeekDates(formatDate(cursor));
      let expectedCount = 0;
      let completedCount = 0;
      const expectedKeySet = new Set<string>();
      weekDates.forEach((date) => {
        const expected = getExpectedExercisesForDate(id || '', date, trainingPlans);
        expectedCount += expected.length;
        expected.forEach(({ planId, exerciseId }) => {
          expectedKeySet.add(`${planId}-${exerciseId}-${date}`);
        });
      });
      const weekCompleted = checkins.filter(
        (c) =>
          c.memberId === id &&
          c.completed &&
          weekDates.includes(c.checkinDate) &&
          expectedKeySet.has(`${c.planId}-${c.exerciseId}-${c.checkinDate}`)
      ).length;
      completedCount = weekCompleted;
      const rate = expectedCount > 0 ? Math.round((completedCount / expectedCount) * 100) : -1;
      result.push({
    ...
    return result;
  }, [memberMeasurements, id, trainingPlans, checkins]);
  ...
  const combinedChartData = useMemo(() => {
    ...
    weeklyCompletionRates.forEach((w) => {
      if (!byDate[w.midDate]) {
        byDate[w.midDate] = { date: w.midDate };
      }
      if (w.completionRate >= 0) {
        byDate[w.midDate].completionRate = w.completionRate;
      }
    });
    return Object.values(byDate).sort((a, b) =>
      (a.date as string).localeCompare(b.date as string)
    );
  }, [memberMeasurements, weeklyCompletionRates, selectedMetrics]);
  ...
  const avgCompletionRate = useMemo(() => {
    const valid = weeklyCompletionRates.filter((w) => w.completionRate >= 0);
    if (valid.length === 0) return -1;
    return Math.round(valid.reduce((s, w) => s + w.completionRate, 0) / valid.length);
  ...
  const showRightAxis =
    selectedMetricConfigs.some((m) => m.yAxis === 'right') || avgCompletionRate >= 0;
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">会员体测趋势</h1>
              <p className="text-sm text-gray-500">
                {member.name} · {member.age}岁 · {member.gender === 'male' ? '男' : '女'}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => navigate(`/coach/members/${member.id}/measurements`)}
            className="flex items-center gap-1.5 px-3 py-2 text-sm font-medium bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 transition-colors"
          >
            <FileText className="w-4 h-4" />
            录入体测
          </button>
          <button
            onClick={() => navigate(`/coach/members/${member.id}/review`)}
            className="flex items-center gap-1.5 px-3 py-2 text-sm font-medium bg-amber-50 text-amber-600 rounded-lg hover:bg-amber-100 transition-colors"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div className="bg-white rounded-2xl p-5 border border-cyan-100 shadow-sm">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-sm text-gray-500">平均训练完成率</p>
          </div>
          <p
            className={cn(
              'text-2xl font-bold',
          ...
          <p className="text-xs text-gray-400 mt-1">
            该时段 {weeklyCompletionRates.filter((w) => w.completionRate >= 0).length} 周有训练安排
          </p>
        </div>
      </div>
              ...
              >
                ...
                {showRightAxis && (
                  <YAxis
                    ...
                    label={{
                      value:
                        selectedMetricConfigs.filter((m) => m.yAxis === 'right').length > 0
                ...
                <Tooltip
                  ...
                  formatter={(value: number, name: string) => {
                    if (name === 'completionRate') return [`${value}%`, '周训练完成率'];
                ...
                <Legend
                  ...
                  formatter={(value) => {
                    if (value === 'completionRate') return '周训练完成率(%)';
                      ...
                      <div className="flex items-center gap-1.5">
                        <span
                          className={cn(
                            ...
                            selectedMetrics.includes(m.key) ? '' : 'bg-gray-300'
                          )}
                          style={{
